...
class Encoder(nn.Module):
    def __init__(self, input_shape=(4, 160, 192, 128)):
        ...
        self.initial_block = nn.Sequential(nn.Conv3d(in_channels=self.input_shape[0], out_channels=32, kernel_size=(3, 3, 3), stride=1),
                                           nn.Dropout3d(p=0.2))

        self.first_encoder = ResNetBlock(in_channel=32, out_channel=32)

        self.second_encoder = nn.Sequential(nn.Conv3d(in_channels=32, out_channels=32, kernel_size=(3, 3, 3), stride=2),
                                            ResNetBlock(in_channel=32, out_channel=64),
                                            ResNetBlock(in_channel=64, out_channel=64))

        self.third_encoder = nn.Sequential(nn.Conv3d(in_channels=64, out_channels=64, kernel_size=(3, 3, 3), stride=2),
                                           ResNetBlock(in_channel=64, out_channel=128),
                                           ResNetBlock(in_channel=128, out_channel=128))

        self.fourth_encoder = nn.Sequential(nn.Conv3d(in_channels=128, out_channels=128, kernel_size=(3, 3, 3), stride=2),
